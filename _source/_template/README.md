# 📁 Template Folder

**Information:**  

✅ Included in backup functionality.  
✅ Files are not overwritten or deleted by updates.  
❌ Do not manually modify or edit files unless you are a developer.  
❌ Files in this folder are not accessible via public links.  

**Folder Description:**  

This folder stores template files downloaded from internal store pages or uploaded manually. It acts as a repository for templates available for installation in the Administrator Module. Templates can be added via store downloads or through the Administrator Interface.

**Documentation:**  

Most documentation for front-end users is located within the Administrator Module across various sections. Developers can access documentation at https://bugfishtm.github.io/suitefish-cms.

🐟 Bugfish
