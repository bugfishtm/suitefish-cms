<?php
	/* 
		...
		File Description:
			Upgrade CMS and Administrator Site Module on request.
			Only possible if administrator module is currently activated 
			and this scripted is executed on page of the administrator module.
	...
	*/ 
	...
	// Only Logged in Superuser can Upgrade
	...
	// Only Hive Mode Administrator Module
	...
	// Check if Upgrades are prevented
	...
	// Do not allow upgrade inside docker container
        ...
        <article>
			<div class="card">
				...
				<?php
					...
					// Recreate internal Cache folder if not exists
					if(is_dir("./_cache/__internal")) { } else { 
						@mkdir("./_cache/__internal"); 
						if(is_dir("./_cache/__internal")) { } 
							else { $error = true; } 
					} 
				?>
				...
				<?php		
					// Set Store URL
					$x = adm_store_curl_string(_HIVE_SERVER_CORE_."/_api/api.store.php?action=cms_latest");
					
					// Check for initial Errors
					if(!$x) {
						?>
						<div class="containererror">
							Error: Cannot reach the online server or fetch a valid latest release!
						</div>
						<?php
						$error = true;
					}
					if(!is_string($x)) {
						?>
						<div class="containererror">
							Error: Online server unreachable or invalid latest release detected!
						</div>
						<?php
						$error = true;
					}				
					...
					if(!$error) {	
						...
						// Operations
						if ($result < 0) {
							// Online version is higher, upgrade possible
							if(file_exists("./_cache/__internal/".$x) AND @$_GET["updt"] == 1 AND $object["csrf"]->check(@$_GET["token"])) {
								if(is_dir("./_cache/__internal/".substr($x, 0, -4)."")) {
									x_rmdir("./_cache/__internal/".substr($x, 0, -4)."");
								}
								$object["zip"]->unzip("./_cache/__internal/".$x."", "./_cache/__internal/".substr($x, 0, -4)."");
								$foldertmp = "./_cache/__internal/".substr($x, 0, -4)."/";				
								// Copy and Replace Folders and Files				
								function upgrader_copyFilesAndFolders($sourceFolder, $destinationFolder) {
									...
									foreach ($items as $item) {
										...
										if (is_dir($sourcePath)) {
											upgrader_copyFilesAndFolders($sourcePath, $destinationPath);  // Recursively copy subdirectories
										} else {
											copy($sourcePath, $destinationPath);
										}
									}
								}				
								// Delete Dirs to Upgrade (To delete old files we delete this folders)
								x_rmdir($object["path"]."/_api");
								x_rmdir($object["path"]."/_core");
								x_rmdir($object["path"]."/_site/_administrator");
								$sourceFolder = "./_cache/__internal/" . substr($x, 0, -4) . "/";
								$destinationFolder = $object["path"]."/";
								upgrader_copyFilesAndFolders($sourceFolder, $destinationFolder); 
								x_rmdir("./_cache/__internal/".substr($x, 0, -4)."");
								unlink("./_cache/__internal/".$x.""); ?>
								<div class="containererrorg">	
									Upgrade successful! The update '<strong><?php echo $x; ?></strong>' has been installed successfully and is now active.
								</div>
								<?php
							} elseif(@$_GET["updt"] == 1) {
								// Download update if requested
								$fileUrl = _HIVE_SERVER_CORE_."/_store/_core/_release/".$x;
								$savePath = './_cache/__internal/'.$x;
								$fp = fopen($savePath, 'w');
								$ch = fopen($fileUrl, 'rb');
								if ($ch === false) { $errorx = true; } else { $errorx = false; }
								while (!feof($ch)) { $data = fread($ch, 8192);  fwrite($fp, $data); }
								fclose($ch);
								fclose($fp);
								// Output
								if ($ch === false) {
									// Upgrade Download Error
									?>
									<div class="containererror">
										Error: The upgrade download failed! Please check the update server availability and ensure the upgrade package is accessible before trying again.
									</div>
									<?php
								} else { ?>
									<div class="containererrorg" style="background: yellow; color: black;">
										The upgrade has been successfully downloaded! 
										This update '<strong><?php echo $x; ?></strong>' is now ready for installation.
									</div>
									<?php
									echo '<br /><a href="./upgrade.php?updt=1&token='.$object["csrf"]->get().'" class="btn">Install</a><br />';
								}
							} else {
								// Show Upgrade available message
								?>
									<div class="containererrorg" style="background: yellow; color: black;">
										An upgrade '<strong><?php echo $x; ?></strong>' is available! 
										This update will upgrade the core CMS and 
										integrated administrator module. No dynamic files will be deleted, but 
										it is recommended to create a backup before upgrading for security purposes. 
									</div>
								<?php
								echo '<br /><a href="./upgrade.php?updt=1" class="btn">Download</a><br />';
							}		
						} elseif ($result > 0) {
							// External Version is lower?
							?>
							<div class="containererror">
								Error: Upgrade failed. Unknown local software version detected!
							</div>
							<?php
						} else {
							...
							?>
							<div class="containererrorg">
								You are currently up to date. No further upgrades are available at this time.
							</div>
							<?php
						}					
					} else {
						?>
						<div class="containererror">
							Upgrade failed due to an unspecified error. This might be caused by a folder or file permission issue.
						</div>
						<?php
					}	
				?>							
