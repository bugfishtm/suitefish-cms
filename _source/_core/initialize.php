	...
	*/ if(!is_array(@$object) OR !is_array($mysql)) { @http_response_code(503); echo "Error [c2]: Your settings.php file is corrupted or has not been included before including initialize.php."; exit(); }	
	...
	#################################################################################################################################################	
		...
		define("_TABLE_API_PERM_",			$object["prefix"]."cms_api_perm");	
			...
			hive___stackecho($object["loadup"]["stack"]);
		define("_TABLE_API_PERM_USER_",		$object["prefix"]."cms_api_perm_user");	
			array_push($object["loadup"]["stack"], array("text" => "'_TABLE_API_PERM_USER_' set to '".$object["prefix"]."cms_api_perm_user"."'"));
			hive___stackecho($object["loadup"]["stack"]);
	...
	#################################################################################################################################################	
		...
		array_push($object["loadup"]["stack"], array("text" => "File Include: '"."/_core/_lib/lib.init.view.php"."'"));
		hive___stackecho($object["loadup"]["stack"]);	
		require_once($object["path"]."/_core/_lib/lib.init.view.php");				
	...
	#################################################################################################################################################		
		if(!_HIVE_PREVENT_INIT_) {
			...
			#################################################################################################################################################	
				...
				hive___stackecho($object["loadup"]["stack"]);
				$object["api_perm_user"] 	= 	new x_class_perm($object["mysql"], _TABLE_API_PERM_USER_, "");
				array_push($object["loadup"]["stack"], array("text" => "'object[api_perm_user] is now x_class_perm"));
				hive___stackecho($object["loadup"]["stack"]);
