	...
	*/ if(!is_array(@$object)) { @http_response_code(503); echo "Startup Error - Please delete your settings.php file and re-install this instance..."; exit(); }	
	...
	#################################################################################################################################################
		define("_HIVE_CREATOR_", 'Powered by Suitefish-CMS');	
			$x11ltmp = array(); $x11ltmp["name"] = "_HIVE_CREATOR_"; $x11ltmp["value"] = _HIVE_CREATOR_;  array_push($object["loadup"]["variables"], $x11ltmp);
	...
	#################################################################################################################################################
		hive__folder_create($object["path"]."/_api", true, false);
		hive__folder_create($object["path"]."/_backup", true, true);
		hive__folder_create($object["path"]."/_cache", true, true);
		hive__folder_create($object["path"]."/_data", true, false);
		hive__folder_create($object["path"]."/_image", true, false);
		hive__folder_create($object["path"]."/_image/__disabled", true, false);
		hive__folder_create($object["path"]."/_site", true, false);
		hive__folder_create($object["path"]."/_site/__disabled", true, false);		
		...
		hive__folder_create($object["path"]."/_template", true, false);
	...
	#################################################################################################################################################
		if(is_array($object["hive_mode"])) {
				...
				hive__folder_create(_HIVE_SITE_PATH_EXT_OFF_, true, false);
				...
				hive__folder_create(_HIVE_SITE_PATH_TH_OFF_, true, false);
		}	
...
Disallow: "._HIVE_URLC_REL_."pkg_sample.php
...
Disallow: "._HIVE_URLC_REL_."upgrade.php");
		}		
...
RewriteRule ^(_template|_cache|_backup) - [F,L]"); }	
