<?php
	...
	*/
	class x_class_api {
		...
		private function create_table() {
			return $this->mysql->query("
				CREATE TABLE IF NOT EXISTS `{$this->table}` (
					...
					`reference` VARCHAR(128) NOT NULL COMMENT 'Reference',
		...
		// Add a new API key (one per reference)
		public function addKey($reference = null, $section = false, $expires_in_days = null, $meta = null) {
			$section = $section ?: $this->section;
			// Check if a key for this reference already exists and is active
			$bind = [
				['type' => 's', 'value' => $reference]
			];
			$existing = $this->mysql->select(
				"SELECT api_key FROM `{$this->table}` WHERE reference = ? AND status = 'active' LIMIT 1",
				false,
				$bind
			);
			if ($existing) {
		...
		// Add a new API key (one per reference)
		public function addKeyNoRef($section = false, $expires_in_days = null, $meta = null) {
			$section = $section ?: $this->section;
			$api_key = $this->generateUniqueKey();
			$expires_at = $expires_in_days ? date('Y-m-d H:i:s', strtotime("+$expires_in_days days")) : null;
			$meta_json = $meta ? json_encode($meta) : null;
		...
		public function refreshKey($reference, $section = false) {
			...
			$updated = $this->mysql->query(
				"UPDATE `{$this->table}` SET api_key = ?, status = 'active', last_used_at = NULL WHERE reference = ? AND status = 'active'",
				$bind
			);
			if (!$updated) {
				return false;
