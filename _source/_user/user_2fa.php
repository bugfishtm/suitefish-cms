<?php
	...
	***********************************************************************************************************/
	if(defined("_HIVE_DEFERSITE_2FA_")) {
		if(_HIVE_DEFERSITE_2FA_) {
			Header("Location: "._HIVE_DEFERSITE_2FA_."");
			exit();
		}
	}
	...
	***********************************************************************************************************/
	if(defined("_HIVE_ENABLESITE_2FA_")) {
		if(!_HIVE_ENABLESITE_2FA_) {
			$object["csrf"] = new x_class_csrf(_HIVE_SITE_COOKIE_."__login_csrf_2fa");
					...
					<div class="row justify-content-center form-bg-image" >
						<div class="col-12 d-flex align-items-center justify-content-center">
							<div class="bg-white shadow border-0 rounded border-light p-4 p-lg-5 w-100 fmxw-500">
								This page is currently deactivated by _HIVE_ENABLESITE_2FA_!
	...
	***********************************************************************************************************/
	if(!_HIVE_INTERNAL_VERSION_ERROR_) {
		...
		foreach ($object["extensions_path"] as $hive_extension_loader_current_init) {
			if (file_exists($hive_extension_loader_current_init."/_user/user_2fa.php")) {
				$object["extension"] = hive__init_extension_header($object, basename($hive_extension_loader_current_init)); 
				require_once $hive_extension_loader_current_init."/_user/user_2fa.php";
				exit();
			}
		} unset($object["extension"]);
		// Sitemod
		if(file_exists($object["path"]."/_site/"._HIVE_MODE_."/_user/user_2fa.php")) { 
			require_once($object["path"]."/_site/"._HIVE_MODE_."/_user/user_2fa.php"); 
			exit();
		} 
	...
	if(hive__trim($xtmp["user_2fa"]) == "") {
		...
		if(@$_POST["fabutton"]) {
			if ($object["csrf"]->check($_POST["token"])) {
				...
				$b = array();
				$b[0]["value"] = $secretKey;
				$object["mysql"]->query("UPDATE "._TABLE_USER_." SET user_2fa = ? WHERE id = '".hive__hsc($object["user"]->user_id)."'", $b);
				$uses_2fa = true;
		...
		if(@$_POST["fabutton"]) {
			if ($object["csrf"]->check($_POST["token"])) {
				$object["mysql"]->query("UPDATE "._TABLE_USER_." SET user_2fa = '' WHERE id = '".hive__hsc($object["user"]->user_id)."'");
				$uses_2fa = false;
			} else { $object["eventbox"]->error($object["lang"]->translate("hive_login_msg_csrf")); }
		}
	}
	$xtmp = $object["user"]->get($object["user"]->user_id);
	hive__default_volt_header($object, $object["lang"]->translate("string_2fa"), $favi_code, "dark");
	?>
		<style>
			/*-----------------------------------------*/
				...
				<div class="col-12 d-flex align-items-center justify-content-center">
					<div class="bg-white shadow border-0 rounded border-light p-4 p-lg-5 w-100 fmxw-500">
						<div class="text-center text-md-center mb-4 mt-md-0">
							<h1 class="mb-0 h3"><?php echo hive__hsc($object["lang"]->translate("string_2fa")); ?></h1>
							...
							<?php } ?>
							<p><?php echo hive__hsc($object["lang"]->translate("hive_login_msg_2fa")); ?></p>
						</div>
						<form action="./user_2fa.php" class="mt-4" method="post">
							<input type="hidden" name="token" value="<?php echo $object["csrf"]->get(); ?>">
							<input type="hidden" name="fabutton" value="1">
							<!-- End of Form -->
							<div class="form-group">
								...
								<div class="form-group mb-4">
									<div class="input-group">
										<div id="twofa_rCode" class="xfpe_width100pct"></div>
									</div>  
								</div>
								<?php
									$xtmp["user_2fa"] = htmlentities(@$xtmp["user_2fa"] ?? '');
									$xtmp["user_2fa"] = "otpauth://totp/Suitefish:?secret=".$xtmp["user_2fa"]."&issuer=Suitefish&digits=6&period=30";
								?>
								<script>
									window.addEventListener("load", () => {
									  const otpUri = "<?php echo $xtmp["user_2fa"]; ?>";
									  new QRCode(document.getElementById("twofa_rCode"), {
										text: otpUri,
										width: 350,
										height: 350
									  });
								...
								<div class="form-group mb-4">
									<label for="email"><?php echo hive__hsc($object["lang"]->translate("string_url")); ?></label>
									<div class="input-group">
										<span class="input-group-text" id="basic-addon1">
											<svg class="icon icon-xs text-gray-600" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M2.003 5.884L10 9.882l7.997-3.998A2 2 0 0016 4H4a2 2 0 00-1.997 1.884z"></path><path d="M18 8.118l-8 4-8-4V14a2 2 0 002 2h12a2 2 0 002-2V8.118z"></path></svg>
										</span>
										<input type="text" name="mail" class="form-control" value="<?php echo $xtmp["user_2fa"]; ?>" placeholder="<?php echo hive__hen($object["lang"]->translate("string_email")); ?>" id="email" autocomplete="off" autofocus readonly>
									</div>  
								</div>
								<?php } ?>
							</div>
							<div class="d-grid">
							<?php if($uses_2fa) { ?>
								<button type="submit" class="btn btn-gray-800"><?php echo hive__hsc($object["lang"]->translate("string_disable")); ?></button> 
							<?php } else { ?>
								<button type="submit" class="btn btn-gray-800"><?php echo hive__hsc($object["lang"]->translate("string_enable")); ?></button>  
							<?php } ?>
