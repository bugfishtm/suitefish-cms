<?php
	...
	***********************************************************************************************************/
	if(defined("_HIVE_DEFERSITE_MAILCHANGE_")) {
		if(_HIVE_DEFERSITE_MAILCHANGE_) {
			Header("Location: "._HIVE_DEFERSITE_MAILCHANGE_."");
			exit();
		}
	}
	...
	***********************************************************************************************************/
	if(defined("_HIVE_ENABLESITE_MAILCHANGE_")) {
		if(!_HIVE_ENABLESITE_MAILCHANGE_) {
			$object["csrf"] = new x_class_csrf(_HIVE_SITE_COOKIE_."__login_csrf_umc");
					...
					<div class="row justify-content-center form-bg-image" > <!-- data-background-lg="../../assets/img/illustrations/signin.svg"-->
						<div class="col-12 d-flex align-items-center justify-content-center">
							<div class="bg-white shadow border-0 rounded border-light p-4 p-lg-5 w-100 fmxw-500">
								This page is currently deactivated by _HIVE_ENABLESITE_MAILCHANGE_!
	...
	***********************************************************************************************************/
	if(!_HIVE_INTERNAL_VERSION_ERROR_) {
		...
		foreach ($object["extensions_path"] as $hive_extension_loader_current_init) {
			if (file_exists($hive_extension_loader_current_init."/_user/user_mail_change.php")) {
				$object["extension"] = hive__init_extension_header($object, basename($hive_extension_loader_current_init)); 
				require_once $hive_extension_loader_current_init."/_user/user_mail_change.php";
				exit();
			}
		} unset($object["extension"]);
		// Sitemod
		if(file_exists($object["path"]."/_site/"._HIVE_MODE_."/_user/user_mail_change.php")) { 
			require_once($object["path"]."/_site/"._HIVE_MODE_."/_user/user_mail_change.php"); 
			exit();
		} 
	} unset($object["hive_mode_config"]); unset($object["extension"]);	
	
	/***********************************************************************************************************
		Mail Change Operations
	***********************************************************************************************************/
	// CSRF for Page
	$object["csrf"] = new x_class_csrf(_HIVE_SITE_COOKIE_."__login_csrf_umc"); 
	...
	if(@$_GET["mai_token"] AND @$_GET["mai_user"]) { 
		...
		***********************************************************************************************************/
		...
		/***********************************************************************************************************
			Request and Template to change Mail
		***********************************************************************************************************/
		if(hive__trim(@$_POST["usermail"]) != "") {
			if($object["csrf"]->check(@$_POST["token"])) {
				...
				if($return == 1) { 
					...
					$url_if_needed = $current_url."/_user/user_mail_change.php?mai_token=".$object["user"]->mail_ref_token."&mai_user=".$object["user"]->mail_ref_user;						
					if(hive__template_mail($object, "SF_DEFAULT_MAILC", $_POST["usermail"], $object["user"]->mail_ref_user, $url_if_needed)) { 
                    ...
                    <div class="col-12 d-flex align-items-center justify-content-center">
                        <div class="bg-white shadow border-0 rounded border-light p-4 p-lg-5 w-100 fmxw-500">
                            <div class="text-center text-md-center mb-4 mt-md-0">
                                <h1 class="mb-0 h3"><?php echo hive__hsc($object["lang"]->translate("hive_login_mc_change_mail")); ?></h1>
								<?php if(!$mail_change_with_token) { ?>
									<p><?php echo hive__hsc($object["lang"]->translate("hive_login_mc_cmailtext")); ?> <b><?php echo hive__hsc($object["user"]->user_mail ?? ''); ?></b></p>
                            ...
                            <form action="./user_mail_change.php" class="mt-4" method="post">
                                ...
                                <div class="form-group mb-4">
                                    <div class="input-group">
										<?php echo hive__hsc($text); ?>
                                    </div>  
                                </div>
								<?php }  ?>
								<?php if(!$mail_change_with_token) { ?>
                                <div class="d-grid">
										<button type="submit" class="btn btn-gray-800"><?php echo hive__hsc($object["lang"]->translate("hive_login_mc_change_mail")); ?></button>
