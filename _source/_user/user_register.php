<?php
	...
	***********************************************************************************************************/
	if(defined("_HIVE_ENABLESITE_REGISTER_")) {
		if(!_HIVE_ENABLESITE_REGISTER_) {
			$object["csrf"] = new x_class_csrf(_HIVE_SITE_COOKIE_."__login_csrf_reg");
	...
	***********************************************************************************************************/
	if(!_HIVE_INTERNAL_VERSION_ERROR_) {
		...
		foreach ($object["extensions_path"] as $hive_extension_loader_current_init) {
			if (file_exists($hive_extension_loader_current_init."/_user/user_register.php")) {
				$object["extension"] = hive__init_extension_header($object, basename($hive_extension_loader_current_init)); 
				require_once $hive_extension_loader_current_init."/_user/user_register.php";
				exit();
			}
		} unset($object["extension"]);
		// Sitemod
		if(file_exists($object["path"]."/_site/"._HIVE_MODE_."/_user/user_register.php")) { 
			require_once($object["path"]."/_site/"._HIVE_MODE_."/_user/user_register.php"); 
			exit();
		} 
	...
	***********************************************************************************************************/
	...
	/***********************************************************************************************************
		Operations for user creation on register
	***********************************************************************************************************/
		$object["csrf"] = new x_class_csrf(_HIVE_SITE_COOKIE_."__login_csrf_reg");
									...
									if($return) { 
										...
										if(is_array($newuser)) {
											...
											if($return) {
												...
												$url_if_needed = $current_url."/_user/user_activate.php?act_token=".$object["user"]->mail_ref_token."&act_user=".$object["user"]->mail_ref_user;
												if(hive__template_mail($object, "SF_DEFAULT_REGISTER", $bind[0]["value"], $object["user"]->mail_ref_user, $url_if_needed)) {
				...
				<div class="col-12 d-flex align-items-center justify-content-center">
					<div class="bg-white shadow border-0 rounded border-light p-4 p-lg-5 w-100 fmxw-500">
						<div class="text-center text-md-center mb-4 mt-md-0">
							<h1 class="mb-0 h3"><?php echo hive__hsc($object["lang"]->translate("string_register")); ?></h1>
						</div>
						<form action="./user_register.php" class="mt-4" method="post">
							...
							<div class="form-group mb-4">
								...
								<div class="input-group">
									...
									<input type="email" name="usermail" autocomplete="off" class="form-control" value="<?php echo hive__hen(@$_POST["usermail"] ?? ''); ?>" placeholder="<?php echo hive__hsc(hive__hen($object["lang"]->translate("string_email"))); ?>" id="email" autofocus required>
								</div>  
							</div>
							<!-- End of Form -->
							<div class="form-group">
								...
								<div class="form-group mb-4">
									...
									<div class="input-group">
										...
										<input type="password" name="password" autocomplete="off" placeholder="<?php echo hive__hen($object["lang"]->translate("string_password")); ?>" class="form-control" id="password" required>
									</div>  
								</div>
								...
								<div class="form-group mb-4">
									...
									<div class="input-group">
										...
										<input type="password" autocomplete="off" name="password_confirm" placeholder="<?php echo hive__hen($object["lang"]->translate("hive_login_password_confirm")); ?>" class="form-control" id="password" required>
									</div>  
								</div>
							</div>
							<div class="d-grid">
								<button type="submit" class="btn btn-gray-800"><?php echo hive__hsc($object["lang"]->translate("string_register")); ?></button>
						...
						<?php if(defined("_HIVE_ENABLESITE_LANGCHANGE_")) {if(_HIVE_ENABLESITE_LANGCHANGE_) { ?>
								<span class="fw-normal mt-0 pt-2 mb-0 pb-0">
								   <?php echo hive__hsc($object["lang"]->translate("hive_login_changelanguage")); ?>
