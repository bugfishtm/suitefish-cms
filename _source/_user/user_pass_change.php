<?php
	...
	***********************************************************************************************************/
	...
	/***********************************************************************************************************
		Defer Site if Required
	***********************************************************************************************************/
	if(defined("_HIVE_DEFERSITE_PASSCHANGE_")) {
		if(_HIVE_DEFERSITE_PASSCHANGE_) {
			Header("Location: "._HIVE_DEFERSITE_PASSCHANGE_."");
			exit();
		}
	}

	/***********************************************************************************************************
		Redirect if Site not Enabled
	***********************************************************************************************************/
	if(defined("_HIVE_ENABLESITE_PASSCHANGE_")) {
		if(!_HIVE_ENABLESITE_PASSCHANGE_) {
			$object["csrf"] = new x_class_csrf(_HIVE_SITE_COOKIE_."__login_csrf_pc");
			hive__default_volt_header($object, $object["lang"]->translate("string_error"), $favi_code, "dark");
			?>
					...
					<div class="row justify-content-center form-bg-image" >
						<div class="col-12 d-flex align-items-center justify-content-center">
							<div class="bg-white shadow border-0 rounded border-light p-4 p-lg-5 w-100 fmxw-500">
								This page is currently deactivated by _HIVE_ENABLESITE_PASSCHANGE_!
								<br /><br /><a href="../"  class="btn btn-gray-200 mt-2"><?php echo hive__hsc($object["lang"]->translate("hive_login_mc_backtohome")); ?></a>
							</div>
						</div>
					</div>
				</div>
	...
	/***********************************************************************************************************
		Overrite with Site Module / Extension File if Present
	***********************************************************************************************************/
	if(!_HIVE_INTERNAL_VERSION_ERROR_) {
		...
		foreach ($object["extensions_path"] as $hive_extension_loader_current_init) {
			if (file_exists($hive_extension_loader_current_init."/_user/user_pass_change.php")) {
				$object["extension"] = hive__init_extension_header($object, basename($hive_extension_loader_current_init)); 
				require_once $hive_extension_loader_current_init."/_user/user_pass_change.php";
				exit();
			}
		} unset($object["extension"]);
		// Sitemod
		if(file_exists($object["path"]."/_site/"._HIVE_MODE_."/_user/user_pass_change.php")) { 
			require_once($object["path"]."/_site/"._HIVE_MODE_."/_user/user_pass_change.php"); 
			exit();
		} 
        ...
        <section class="vh-lg-100 mt-5 mt-lg-0 bg-soft d-flex align-items-center mb-4">
            <div class="container mb-5">
                <div class="row justify-content-center form-bg-image" >
                    <div class="col-12 d-flex align-items-center justify-content-center">
                        <div class="bg-white shadow border-0 rounded border-light p-4 p-lg-5 w-100 fmxw-500">
							<?php echo hive__hsc($object["lang"]->translate("hive_cannotenterwhilenologin")); ?>
							<br /><br /><a href="../" class="btn btn-gray-200 mt-2"><?php echo hive__hsc($object["lang"]->translate("hive_login_mc_backtohome")); ?></a>
                        </div>
                    </div>
                </div>
	...
	***********************************************************************************************************/
	$object["csrf"] = new x_class_csrf(_HIVE_SITE_COOKIE_."__login_csrf_pc"); 	
	if(hive__trim(@$_POST["token"]) != "") {
		if(hive__trim(@$_POST["passwordc"]) != "") {
			if(hive__trim(@$_POST["password"]) != "") {
					...
					if($object["csrf"]->check(@$_POST["token"])) {
						if($_POST["password"] == $_POST["password_confirm"]) {
							if($object["user"]->passfilter_check($_POST["password_confirm"])) { 
								$xtmp = $object["user"]->get($object["user"]->user_id);
								if($object["user"]->password_check($_POST["passwordc"], $xtmp["user_pass"])) {
									$object["user"]->changeUserPass($object["user"]->user_id, $_POST["password_confirm"]);
									$object["eventbox"]->ok($object["lang"]->translate("hive_login_msg_passc_ok"));
								} else { $object["eventbox"]->error($object["lang"]->translate("hive_login_msg_passc_cwrong")); }
				...
				} else { $object["eventbox"]->error($object["lang"]->translate("hive_login_msg_passc_enternewc")); }
			} else { $object["eventbox"]->error($object["lang"]->translate("hive_login_msg_passc_enternew")); }
		} else { $object["eventbox"]->error($object["lang"]->translate("hive_login_msg_passc_enterold")); }
	}
	
	hive__default_volt_header($object, $object["lang"]->translate("string_change_password"), $favi_code, "dark"); ?>
		<style>
			/*-----------------------------------------*/
                    ...
                    <div class="col-12 d-flex align-items-center justify-content-center">
                        <div class="bg-white shadow border-0 rounded border-light p-4 p-lg-5 w-100 fmxw-500">
                            <div class="text-center text-md-center mb-4 mt-md-0">
                                <h1 class="mb-0 h3"><?php echo hive__hsc($object["lang"]->translate("string_change_password")); ?></h1>
								<?php if($object["user"]->login_as_is()) { ?>
									<div class="alert alert-danger"><?php echo hive__hsc($object["lang"]->translate("hive_login_msg_passc_loginasact")); ?></div>
								<?php } ?>
                            </div>
								<b><?php echo $object["lang"]->translate("hive_login_password_rules"); ?></b>
								<ul>
									<?php if (_USER_PF_SIGNS_ > 0) { ?><li><?php echo hive__hsc($object["lang"]->translate("hive_login_password_sign")); ?> <?php echo _USER_PF_SIGNS_; ?></li><?php } ?>
									<?php if (_USER_PF_CAPSIGNS_ > 0) { ?><li><?php echo hive__hsc($object["lang"]->translate("hive_login_password_cap")); ?> <?php echo _USER_PF_CAPSIGNS_; ?></li><?php } ?>
									<?php if (_USER_PF_SMSIGNS_ > 0) { ?><li><?php echo hive__hsc($object["lang"]->translate("hive_login_password_small")); ?> <?php echo _USER_PF_SMSIGNS_; ?></li><?php } ?>
									<?php if (_USER_PF_SPSIGNS_ > 0) { ?><li><?php echo hive__hsc($object["lang"]->translate("hive_login_password_special")); ?> <?php echo _USER_PF_SPSIGNS_; ?></li><?php } ?>
									<?php if (_USER_PF_NUMSIGNS_ > 0) { ?><li><?php echo hive__hsc($object["lang"]->translate("hive_login_password_numeric")); ?> <?php echo _USER_PF_NUMSIGNS_; ?></li><?php } ?>
								</ul>
                            <form action="./user_pass_change.php" class="mt-4" method="post">
								<input type="hidden" name="token" value="<?php echo $object["csrf"]->get(); ?>">
								<input type="hidden" name="loginbutton" value="1">
                                <!-- Form -->
								<div class="form-group mb-4">
									<label for="password"><?php echo hive__hsc($object["lang"]->translate("string_current_password")); ?></label>
									<div class="input-group">
										<span class="input-group-text" id="basic-addon2">
											<svg class="icon icon-xs text-gray-600" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clip-rule="evenodd"></path></svg>
										</span>
										<input type="password" autocomplete="off" placeholder="<?php echo hive__hen($object["lang"]->translate("string_current_password")); ?>" class="form-control" id="password" name="passwordc" required>
									</div>  
								</div>
                                <!-- Form -->
								<div class="form-group mb-4">
									<label for="password"><?php echo hive__hsc($object["lang"]->translate("string_new_password")); ?></label>
									<div class="input-group">
										<span class="input-group-text" id="basic-addon2">
											<svg class="icon icon-xs text-gray-600" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clip-rule="evenodd"></path></svg>
										</span>
										<input type="password" autocomplete="off" placeholder="<?php echo hive__hen($object["lang"]->translate("string_new_password")); ?>" class="form-control" id="password" name="password" required>
									</div>  
								</div>
								<!-- Form -->
								<div class="form-group mb-4">
									<label for="password"><?php echo hive__hsc($object["lang"]->translate("string_confirm_new_password")); ?></label>
									<div class="input-group">
										<span class="input-group-text" id="basic-addon2">
											<svg class="icon icon-xs text-gray-600" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clip-rule="evenodd"></path></svg>
										</span>
										<input type="password" autocomplete="off" placeholder="<?php echo hive__hen($object["lang"]->translate("string_confirm_new_password")); ?>" class="form-control" id="password" name="password_confirm" required>
									</div>  
								</div>
                                <div class="d-grid">
									<button type="submit" class="btn btn-gray-800"><?php echo hive__hsc($object["lang"]->translate("string_change_password")); ?></button>
									<a href="../" class="btn btn-gray-200 mt-2"><?php echo hive__hsc($object["lang"]->translate("hive_login_mc_backtohome")); ?></a>
                                </div>
                            </form>
                        </div>
                    </div>
	...
	<?php
		$object["eventbox"]->show(hive__hsc($object["lang"]->translate("string_close")), true);
		hive__default_volt_footer($object, "");
	?>