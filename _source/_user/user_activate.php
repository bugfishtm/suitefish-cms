<?php
	...
	***********************************************************************************************************/
	if(defined("_HIVE_DEFERSITE_ACTIVATE_")) {
		if(_HIVE_DEFERSITE_ACTIVATE_) {
			Header("Location: "._HIVE_DEFERSITE_ACTIVATE_."");
			exit();
		}
	}
	...
	***********************************************************************************************************/
	if(defined("_HIVE_ENABLESITE_ACTIVATE_")) {
		if(!_HIVE_ENABLESITE_ACTIVATE_) {
			$object["csrf"] = new x_class_csrf(_HIVE_SITE_COOKIE_."__login_csrf_act");
					...
					<div class="row justify-content-center form-bg-image" >
						<div class="col-12 d-flex align-items-center justify-content-center">
							<div class="bg-white shadow border-0 rounded border-light p-4 p-lg-5 w-100 fmxw-500">
								This page is currently deactivated by _HIVE_ENABLESITE_ACTIVATE_!
	...
	***********************************************************************************************************/
	if(!_HIVE_INTERNAL_VERSION_ERROR_) {
		...
		foreach ($object["extensions_path"] as $hive_extension_loader_current_init) {
			if (file_exists($hive_extension_loader_current_init."/_user/user_activate.php")) {
				$object["extension"] = hive__init_extension_header($object, basename($hive_extension_loader_current_init)); 
				require_once $hive_extension_loader_current_init."/_user/user_activate.php";
				exit();
			}
		} unset($object["extension"]);
		// Sitemod
		if(file_exists($object["path"]."/_site/"._HIVE_MODE_."/_user/user_activate.php")) { 
			require_once($object["path"]."/_site/"._HIVE_MODE_."/_user/user_activate.php"); 
			exit();
		} 
	} unset($object["hive_mode_config"]); unset($object["extension"]);	

	/***********************************************************************************************************
		Output Error ig already logged in!
	***********************************************************************************************************/
	...
	***********************************************************************************************************/
	...
	/***********************************************************************************************************
		Activation Request Operations and Default Template
	***********************************************************************************************************/
	$object["csrf"] = new x_class_csrf(_HIVE_SITE_COOKIE_."__login_csrf_act");
	$return = hive__template_user_activate($object, "act_token", "act_user", false, false);
	hive__default_volt_header($object, $object["lang"]->translate("hive_login_title_accactivation"), $favi_code, "dark");
		$text = "";
