<?php
	...
	***********************************************************************************************************/
	if(defined("_HIVE_DEFERSITE_LANGCHANGE_")) {
		if(_HIVE_DEFERSITE_LANGCHANGE_) {
			Header("Location: "._HIVE_DEFERSITE_LANGCHANGE_."");
			exit();
		}
	}
	...
	***********************************************************************************************************/
	if(defined("_HIVE_ENABLESITE_LANGCHANGE_")) {
		if(!_HIVE_ENABLESITE_LANGCHANGE_) {
			$object["csrf"] = new x_class_csrf(_HIVE_SITE_COOKIE_."__login_csrf_lang");
					...
					<div class="row justify-content-center form-bg-image" > <!-- data-background-lg="../../assets/img/illustrations/signin.svg"-->
						<div class="col-12 d-flex align-items-center justify-content-center">
							<div class="bg-white shadow border-0 rounded border-light p-4 p-lg-5 w-100 fmxw-500">
								This page is currently deactivated by _HIVE_ENABLESITE_LANGCHANGE_!
	...
	***********************************************************************************************************/
	if(!_HIVE_INTERNAL_VERSION_ERROR_) {
		...
		foreach ($object["extensions_path"] as $hive_extension_loader_current_init) {
			if (file_exists($hive_extension_loader_current_init."/_user/user_language.php")) {
				$object["extension"] = hive__init_extension_header($object, basename($hive_extension_loader_current_init)); 
				require_once $hive_extension_loader_current_init."/_user/user_language.php";
				exit();
			}
		} unset($object["extension"]);
		// Sitemod
		if(file_exists($object["path"]."/_site/"._HIVE_MODE_."/_user/user_language.php")) { 
			require_once($object["path"]."/_site/"._HIVE_MODE_."/_user/user_language.php"); 
			exit();
		} 
	...
	***********************************************************************************************************/
	$object["csrf"] = new x_class_csrf(_HIVE_SITE_COOKIE_."__login_csrf_lang");
	if(!defined("_HIVE_LANG_ARRAY_")) { hive__error("Access Error", "There are no languages available to be selected!", "", true, 403); }
	if(!is_array(_HIVE_LANG_ARRAY_)) { hive__error("Access Error", "There are no languages available to be selected!", "", true, 403); }	
	if(isset($_GET["change"])) {  
		...
		Header("Location: ./user_language.php");
		exit();			
	}		
			...
			<div class="col-12 d-flex align-items-center justify-content-center">
				<div class="bg-white shadow border-0 rounded border-light p-4 p-lg-5 w-100 fmxw-500">
					<div class="text-center text-md-center mb-4 mt-md-0">
						<h1 class="mb-0 h3"><?php echo hive__hsc($object["lang"]->translate("hive_login_change_Lang")); ?></h1>
					</div>
					<!-- Form -->
					<div class="form-group mb-4">
						<?php foreach(_HIVE_LANG_ARRAY_ as $key => $value) { ?>	
							<a class="d-flex align-items-center" href="./user_language.php?change=<?php echo hive__hsc($value ?? ''); ?>">
							 <div><?php 
									if ($value == "en") { echo "English"; }
									elseif ($value == "de") { echo "Deutsch"; }
									elseif ($value == "es") { echo "Español"; } 
									elseif ($value == "fr") { echo "Français"; } 
									elseif ($value == "it") { echo "Italiano"; } 
									elseif ($value == "zh") { echo "中文"; } 
									elseif ($value == "ja") { echo "日本語"; }
									elseif ($value == "ru") { echo "Русский"; }
									elseif ($value == "pt") { echo "Português"; } 
									elseif ($value == "ar") { echo "العربية"; } 
									elseif ($value == "in") { echo "हिन्दी"; } 
									elseif ($value == "ru") { echo "Русский"; }	
									elseif ($value == "tr") { echo "Türkçe"; }				
									elseif ($value == "zh") { echo "中文"; }
									elseif ($value == "kr") { echo "한국어"; }
									else { echo hive__hsc($value ?? ''); }
							 ?></div>
							</a>
						<?php } ?>	
					</div>
					<div class="d-grid">
						<?php if(defined("_HIVE_ENABLESITE_LOGIN_") AND !$object["user"]->user_loggedIn) { if(_HIVE_ENABLESITE_LOGIN_) { ?> <a href="./user_login.php" class="btn btn-gray-200 mt-2"><?php echo hive__hsc($object["lang"]->translate("hive_login_backtologin")); ?></a> <?php } } ?>
