<?php
	...
	***********************************************************************************************************/
	if(defined("_HIVE_DEFERSITE_MODESWITCH_")) {
		if(_HIVE_DEFERSITE_MODESWITCH_) {
			Header("Location: "._HIVE_DEFERSITE_MODESWITCH_."");
			exit();
		}
	}
	...
	***********************************************************************************************************/
	if(defined("_HIVE_ENABLESITE_MODESWITCH_")) {
		if(!_HIVE_ENABLESITE_MODESWITCH_) {
			$object["csrf"] = new x_class_csrf(_HIVE_SITE_COOKIE_."__login_csrf_switch");
					...
					<div class="row justify-content-center form-bg-image" >
						<div class="col-12 d-flex align-items-center justify-content-center">
							<div class="bg-white shadow border-0 rounded border-light p-4 p-lg-5 w-100 fmxw-500">
								This page is currently deactivated by _HIVE_ENABLESITE_MODESWITCH_!
	...
	***********************************************************************************************************/ 
	if(!_HIVE_INTERNAL_VERSION_ERROR_) {
		...
		foreach ($object["extensions_path"] as $hive_extension_loader_current_init) {
			if (file_exists($hive_extension_loader_current_init."/_user/user_switch.php")) {
				$object["extension"] = hive__init_extension_header($object, basename($hive_extension_loader_current_init)); 
				require_once $hive_extension_loader_current_init."/_user/user_switch.php";
				exit();
			}
		} unset($object["extension"]);
		// Sitemod 
		if(file_exists($object["path"]."/_site/"._HIVE_MODE_."/_user/user_switch.php")) { 
			require_once($object["path"]."/_site/"._HIVE_MODE_."/_user/user_switch.php"); 
			exit();
		} 
	} unset($object["hive_mode_config"]); unset($object["extension"]);	

	/***********************************************************************************************************
		Switch between Front and Backend
	***********************************************************************************************************/
    ...
    <body>
        <article><div class="card">
            <h1>Backend/Frontend Switch</h1>
				<?php if(@$_SESSION[_HIVE_COOKIE_."hive_mode"] == _HIVE_ADMIN_SITE_ OR !_HIVE_ADMIN_SITE_) { $is_admin = true; } else { $is_admin = false; } ?>
				...
				<?php if($is_admin) { 
					...
					?>
						<input type="submit" value="<?php echo hive__hen($object["lang"]->translate("string_switch")); ?>" name="adm_change_access" class="btn"> <a href="../" class="btn"><?php echo hive__hsc($object["lang"]->translate("hive_login_mc_backtohome")); ?></a><br />
					</form>
				<?php } else { 
					if(@$_GET["adm_change_access"]) {
